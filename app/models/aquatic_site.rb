...
class AquaticSite < AquaticDataWarehouse::BaseTbl
  # ASSOCIATIONS
  belongs_to :waterbody, :foreign_key => 'waterbody_id'
  ...
  alias_attribute :raw_latitude,  :x_coordinate
  alias_attribute :raw_longitude, :y_coordinate
  
  def drainage_code
    self.waterbody.drainage_code if self.waterbody
  end    
  
  # throw error if delete is attempted when the site is in use by any agency    
