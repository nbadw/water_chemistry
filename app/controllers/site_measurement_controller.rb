class SiteMeasurementController < ApplicationController
  before_filter :calculate_substrate_accounted_for
  ...
  def calculate_substrate_accounted_for    
    @substrate_accounted_for = SiteMeasurement.substrate_accounted_for active_scaffold_session_storage[:constraints][:aquatic_activity_event_id]
  end
end
